...
def execute(data):
    ...
    minX, maxX, minY, maxY = minmax(
        layers[1])  # Note: layers[0] contains Cura start data, the first layer is contained in layers[1]
    ...
    for layer in layers[1:]:
        ...
        if layers.index(layer) >= 1:  # Add nonexistant, not-executed command for search functionalities to default to
