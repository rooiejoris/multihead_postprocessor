...
#maxprintrange = 200
#minObjBCWidth = nozzleoffset - maxprintrange  # 115
#totObjWidth = 516.289 #350
externalxcurrent = 0
stepfactor = 400/836
slaveyoffset = 0                                   # Y offset of slave
f_current = 1800
primenozzlegcode = "G1 E13.8 F2700\n\n"
...
def initlayer(objectminx, objectmaxx):
    ...
    if (domains[7] > machinerange and blindspotcorrected == False):
        ...
        print("Notice! Correcting this layer by moving both the right and left positions to the left to respect rightmost machine boundary")
        domains[7] = machinerange
        domains[6] = domains[7] - printrangex
        domains[3] = domains[7] - nozzleoffset
        domains[2] = domains[3] - printrangex
    ...
    global cutlines
    cutlines = [cut_ab - overlap/2, cut_ab + overlap/2, cut_bc - overlap/2, cut_bc + overlap/2, cut_cd - overlap/2, cut_cd + overlap/2]
    return domains

# Create toolchangescripts for this layer
def layerproperties(objectminx, objectmaxx):
...
def checkduplicate(line, sublayer):
    ...
    if (line.find("X") == -1 and line.find("Y") and line.find("E") != -1):
        isretraction = True
    else: isretraction = False

    if (line.find("X") == -1):                            #Note the current X and Y positions, and compare
        currentx = lastx
    else: currentx = getValue(line, "X", None)
    if line.find("Y") == -1:
        currenty = lasty
    else: currenty = getValue(line, "Y", None)
    ...
    for currentline in totalgcode:
        ...
        if "E" in currentline:             # Look for lines with extrusion commands
            if currentline.find("G92")>=0:    #If line is a G92, use G92 value as previous E
                ...
                if e_value != None:
                    e_previous = e_value
            elif (currentline.find('E') < currentline.find(';')) or (currentline.find("E") > 0 and currentline.find(';') == -1): #Only process line if extrusion is not a comment
                ...
                currentline = re.sub("E\d+[.]\d+", "E"+str(e_relative),currentline)        # Replace old E value with new E value
...
def fetchlayer(totalgcode):
    ...
    currentlayer = []
    for currentline in totalgcode:
        if currentline.find(";LAYER:") >= 0:        #";LAYER" is a layer change script used in the standard setup of Cura (current version:2.7.0), might need adjustment in the future
...
def cutlinecrossed(x_current, x_previous, cutlines):
    ...
    for value in cutlines:
        if x_current <= value and x_previous <= value:          linescrossed.append(False)
        elif x_current <= value and x_previous >= value:        linescrossed.append(True)
        elif x_current >= value and x_previous <= value:        linescrossed.append(True)
        elif x_current >= value and x_previous >= value:        linescrossed.append(False)
        else: print("Cutline check "+ value + "Failed!")
    return linescrossed
...
def updateeconstant(x_current, y_current, e_current, x_previous, y_previous):
    ...
    if length == 0 or e_current == 0:
        econstant = econstant
    else: econstant = e_current/length
...
def courier(line, x_left, x_right):
    ...
    else: print("Courier failed! " + str(line) +"(x_left: "+str(x_left)+" x_right: "+str(x_right) + ")\n")
...
def cutter(linetocut, cutline, x_current, y_current, x_previous, y_previous):
    ...
    if min(x_current, x_previous) == x_previous:
        ...
        x_r = x_previous
        y_r = y_previous
    else: print('Aligning line from left to right in the cutter went wrong! How is that even possible?')
...
def distributor(layer):
    ...
    for line in layer:
        ...
        if (comment_pos == -1 and z_pos != -1) or ((comment_pos > z_pos) and z_pos != -1):
            ...
            else: print("Distribution error: non-movement command not distributed")
        ...
        # Line is not a comment and contains XY movement coordinates (should only be G0 or G1)
            ...
            # Only the part right of any cutline should remain at this point, the courier will place it in the correct sublayer
                ...
                else: courier(line, x_current, x_previous)
...
def recombinelayer(sublayer_a, sublayer_b, sublayer_c, sublayer_d):
    ...
    mastertempfile.append(toolchangeMasterRight)
    for b in sublayer_b:
        mastertempfile.append(b)
    mastertempfile.append(toolchangeMasterLeft)
    for c in sublayer_c:
        slavetempfile.append(c)
    slavetempfile.append(toolchangeSlaveRight)
    for d in sublayer_d:
        slavetempfile.append(d)
    slavetempfile.append(toolchangeSlaveLeft)
...
print("File read completed! Time to read file: "+str(timefilereader - starttime)+ " Total time elapsed: " + str(timefilereader-starttime))
...
layers = fetchlayer(filereader.totalgcode)      # Extract layers
timelayers = time.time()
print("Layers extracted! Time to extractlayers: "+str(timelayers - timeminmax)+ " Total time elapsed: " + str(timelayers-starttime))
minX, maxX, minY, maxY  = minmax(layers[1])     # Note: layers[0] contains Cura start data, the first layer is contained in layers[1]
layerproperties(minX, maxX)
...
# Process each layer using distributor and recombinelayer methods for each layer
for layer in layers[1:]:
    minX, maxX, minY, maxY = minmax(layer)
    layerproperties(minX, maxX)
...
print("Distributor complete! Time to distibute lines: "+str(timedistributor - timelayers)+ " Total time elapsed: " + str(timedistributor-starttime))

mastertempfile.append(endscriptMaster)
...
timeend = time.time()
print("File write complete! Time to write file: "+str(timeend - timedistributor)+ " Total time elapsed: " + str(timeend-starttime))
